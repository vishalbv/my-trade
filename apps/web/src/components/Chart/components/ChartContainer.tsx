...
import { TimeframeConfig, Drawing, OHLCData } from "../types";
...
const calculateSwingAreas = (chartData: OHLCData[]): Drawing[] => {
  ...
  const isSwingLow = (index: number, data: OHLCData[]): boolean => {
    if (index < 15 || index > data.length - 15) return false;

    const currentLow = data[index]?.low;

    for (let i = index - 15; i <= index + 15; i++) {
      if (i === index) continue;
      if (data[i]?.low! < currentLow!) return false;
    }
    return true;
  };
...
export const ChartContainer = memo(
  ({
    ...
    // Combine regular drawings with swing area drawings
    const allDrawings = useMemo(() => {
      return [...symbolDrawings, ...swingDrawings];
    }, [symbolDrawings, swingDrawings]);
